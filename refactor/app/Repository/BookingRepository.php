    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!empty($jobs)) {
            foreach ($jobs as $jobitem) {
                $groupedJobs = ($jobitem->immediate == 'yes') ? 'emergencyJobs' : 'noramlJobs';
                ${$groupedJobs}[] = $jobitem;
            }
    ...
    public function store($user, $data)
{
    ...
    if (empty($data['from_language_id'])) {
        return $this->failResponse("Du måste fylla in alla fält", "from_language_id");
    }

    if ($data['immediate'] == 'no' && empty($data['due_date']) && empty($data['due_time'])) {
        return $this->failResponse("Du måste fylla in alla fält", "due_date");
    }

    if ($data['immediate'] == 'no' && empty($data['due_time'])) {
        return $this->failResponse("Du måste fylla in alla fält", "due_time");
    }

    if ($data['immediate'] == 'no' && empty($data['customer_phone_type']) && empty($data['customer_physical_type'])) {
        return $this->failResponse("Du måste göra ett val här", "customer_phone_type");
    }

    if ($data['immediate'] == 'no' && empty($data['duration'])) {
        return $this->failResponse("Du måste fylla in alla fält", "duration");
    }
    ...
    if ($data['immediate'] == 'yes') {
        ...
        if ($due_carbon->isPast()) {
            return $this->failResponse("Can't create booking in past");
        }
    }

    $this->setCertifiedType($data);
    ...
    $response = [
        'status' => 'success',
        'id' => $job->id,
        'job_for' => $this->getJobForLabels($job),
...
protected function failResponse($message, $field_name = null)
{
    return [
        'status' => 'fail',
        'message' => $message,
...
protected function setCertifiedType(&$data)
{
    $jobFor = $data['job_for'];

    if (in_array('certified', $jobFor) && in_array('normal', $jobFor)) {
        $data['certified'] = 'both';
    ...
    if (in_array('normal', $jobFor) && in_array('certified_in_law', $jobFor)) {
...
protected function getJobType($consumer_type)
{
...
protected function getJobForLabels($job)
{
    $labels = [];

    if ($job->gender != null) {
        $labels[] = ($job->gender == 'male') ? 'Man' : 'Kvinna';
    }
    ...
    if ($job->certified != null) {
        if ($job->certified == 'both') {
            ...
            $labels[] = ($job->certified == 'yes') ? 'certified' : $job->certified;
        }
    }
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $user = $job_detail->user ?: $job_detail->user()->first();
        ...
        Event::fire(new SessionEnded($job_detail, ($post_data['userid'] == $job_detail->user_id) ? $translator->user_id : $job_detail->user_id));
    ...
    public function getPotentialJobIdsWithUserId($user_id)
{
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            if (!$this->isNeedToSendPush($oneUser->id) || $data['immediate'] == 'yes' && TeHelper::getUsermeta($oneUser->id, 'not_get_emergency') == 'yes') {
                continue;
            }
